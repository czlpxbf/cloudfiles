...
import { runCommand, now } from './utils.js';
...
export async function deployIndex(jsonData) {
  ...
  fs.writeFileSync(path.join(siteDir, 'main.json'), JSON.stringify(jsonData, null, 2));

  console.log('\n开始部署...');
  await runCommand('wrangler', ['pages', 'deploy', siteDir, `--project-name=${MAIN_PROJECT_NAME}`, '--branch=main']);
}
