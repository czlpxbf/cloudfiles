...
/**
 * 批量部署多个文件到 Pages 项目（共享一次部署）
 * 所有文件共享一个 JWT 和一个 deployment，大幅减少 API 调用
 *
 * @param {Array<{filePath: string, remoteFileName: string}>} files - 文件列表
 * @param {string} projectName - Pages 项目名
 * @returns {Promise<Array<string>>} 每个文件的 URL
 */
export async function deployFiles(files, projectName) {
  const accountId = await getAccountId();
  ...
  // 准备所有文件的数据
  for (const { filePath, remoteFileName } of files) {
    ...
    console.log(`           API 上传: ${path.basename(filePath)} (${(fileBuffer.length / 1024).toFixed(1)} KB)`);
  }

  // Step 2: 批量上传所有文件（一次 API 调用）
  console.log(`           上传 ${files.length} 个文件到 Pages...`);
  await uploadAssets(jwt, uploadItems);

  // Step 3: 批量注册哈希（一次 API 调用）
  await upsertHashes(jwt, hashes);

  // Step 4: 创建部署（一次 API 调用，包含所有文件）
  console.log(`           创建部署 (${files.length} 个文件)...`);
  const deployResult = await createDeployment(accountId, projectName, manifest);

  // 构建每个文件的 URL
  const projectSubdomain = deployResult.result?.project_subdomain;
  const baseUrl = projectSubdomain
    ? `https://${projectSubdomain}.pages.dev`
    : `https://${projectName}.pages.dev`;

  const urls = files.map(f => `${baseUrl}/${f.remoteFileName}`);
